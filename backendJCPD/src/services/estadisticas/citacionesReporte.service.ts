 ...
 */
export async function contarcitacionesTotales(filtro: FiltroReporte) {
  ...
  const total = await Citacion.count({
  ...
  const oficio = await Citacion.count({
    where: whereNotificacion,
    include: [
      {
        model: Denuncia,
  ...
  const externa = await Citacion.count({
    where: whereNotificacion,
    include: [
      {
        model: Denuncia,
